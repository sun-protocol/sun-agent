...
class AssistantAgent(BaseChatAgent, Component[AssistantAgentConfig]):
    ...
    The :meth:`on_messages` returns a :class:`~sunagent_agentchat.base.Response`
    in which :attr:`~sunagent_agentchat.base.Response.chat_message` is the final
    response message.

    The :meth:`on_messages_stream` creates an async generator that produces
    the inner messages as they are created, and the :class:`~sunagent_agentchat.base.Response`
    ...
    * If the model returns no tool call, then the response is immediately returned as a :class:`~sunagent_agentchat.messages.TextMessage` in :attr:`~sunagent_agentchat.base.Response.chat_message`.
    * When the model returns tool calls, they will be executed right away:
        - When `reflect_on_tool_use` is False (default), the tool call results are returned as a :class:`~sunagent_agentchat.messages.ToolCallSummaryMessage` in :attr:`~sunagent_agentchat.base.Response.chat_message`. `tool_call_summary_format` can be used to customize the tool call summary.
        - When `reflect_on_tool_use` is True, the another model inference is made using the tool calls and results, and the text response is returned as a :class:`~sunagent_agentchat.messages.TextMessage` in :attr:`~sunagent_agentchat.base.Response.chat_message`.
    * If the model returns multiple tool calls, they will be executed concurrently. To disable parallel tool calls you need to configure the model client. For example, set `parallel_tool_calls=False` for :class:`~sunagent_ext.models.openai.OpenAIChatCompletionClient` and :class:`~sunagent_ext.models.openai.AzureOpenAIChatCompletionClient`.
    ...
    * If a handoff is triggered, a :class:`~sunagent_agentchat.messages.HandoffMessage` will be returned in :attr:`~sunagent_agentchat.base.Response.chat_message`.
    * If there are tool calls, they will also be executed right away before returning the handoff.
    * The tool calls and results are passed to the target agent through :attr:`~sunagent_agentchat.messages.HandoffMessage.context`.
    ...
    the `model_context` parameter to a :class:`~sunagent_core.model_context.BufferedChatCompletionContext`.
    ...
    :class:`~sunagent_core.model_context.ChatCompletionContext`.
    ...
    :class:`~sunagent_agentchat.messages.ModelClientStreamingChunkEvent`
    ...
    Args:
        ...
        handoffs (List[HandoffBase | str] | None, optional): The handoff configurations for the agent,
            ...
            The transfer is only executed when the team is in :class:`~sunagent_agentchat.teams.Swarm`.
            If a handoff is a string, it should represent the target agent's name.
        model_context (ChatCompletionContext | None, optional): The model context for storing and retrieving :class:`~sunagent_core.models.LLMMessage`. It can be preloaded with initial messages. The initial messages will be cleared when the agent is reset.
        ...
        model_client_stream (bool, optional): If `True`, the model client will be used in streaming mode.
            :meth:`on_messages_stream` and :meth:`BaseChatAgent.run_stream` methods will also yield :class:`~sunagent_agentchat.messages.ModelClientStreamingChunkEvent`
    ...
    Examples:
        ...
        **Example 1: basic agent**
        ...
        .. code-block:: python
            ...
            import asyncio
            from sunagent_core import CancellationToken
            from sunagent_ext.models.openai import OpenAIChatCompletionClient
            from sunagent_agentchat.agents import AssistantAgent
            from sunagent_agentchat.messages import TextMessage
        ...
        **Example 3: agent with tools**
        ...
        print the messages to the console using :class:`~sunagent_agentchat.ui.Console`.

        The tool is a simple function that returns the current time.
        Under the hood, the function is wrapped in a :class:`~sunagent_core.tools.FunctionTool`
        ...
        .. code-block:: python
            ...
            from sunagent_agentchat.agents import AssistantAgent
            from sunagent_agentchat.messages import TextMessage
            from sunagent_agentchat.ui import Console
            from sunagent_core import CancellationToken
        ...
        **Example 4: agent with structured output and tool**
        ...
        Note that you need to use :class:`~sunagent_core.tools.FunctionTool` to create the tool
        ...
        .. code-block:: python
            ...
            from sunagent_agentchat.ui import Console
            from sunagent_core import CancellationToken
            from sunagent_core.tools import FunctionTool
            from sunagent_ext.models.openai import OpenAIChatCompletionClient
            from pydantic import BaseModel
        ...
        **Example 5: agent with bounded model context**
        ...
        :class:`~sunagent_core.model_context.BufferedChatCompletionContext`
        ...
        .. code-block:: python
            ...
            from sunagent_agentchat.messages import TextMessage
            from sunagent_core import CancellationToken
            from sunagent_core.model_context import BufferedChatCompletionContext
            from sunagent_ext.models.openai import OpenAIChatCompletionClient
        ...
        **Example 6: agent with memory**
        ...
        before making an inference, using the :meth:`~sunagent_core.memory.Memory.update_context` method.
        ...
        .. code-block:: python
            ...
            from sunagent_agentchat.messages import TextMessage
            from sunagent_core import CancellationToken
            from sunagent_core.memory import ListMemory, MemoryContent
            from sunagent_ext.models.openai import OpenAIChatCompletionClient
        ...
        **Example 8: agent using reasoning model with custom model context.**
        ...
        .. code-block:: python
            ...
            from sunagent_agentchat.agents import AssistantAgent
            from sunagent_core.model_context import UnboundedChatCompletionContext
            from sunagent_core.models import AssistantMessage, LLMMessage, ModelFamily
            from sunagent_ext.models.ollama import OllamaChatCompletionClient
    ...
    component_provider_override = "sunagent_agentchat.agents.AssistantAgent"
    ...
    def __init__(
        ...
        *,
        ...
        reflection_system_message: (str | None) = None,
        model_client_stream: bool = False,
        ...
        if reflect_on_tool_use and ModelFamily.is_claude(model_client.model_info["family"]):
            warnings.warn(
                ...
                "See [Single-Agent Team](https://microsoft.github.io/sunagent/stable/user-guide/agentchat-user-guide/tutorial/teams.html#single-agent-team) "
                "for more details.",
                UserWarning,
                stacklevel=2,
            )
        ...
        self._reflection_system_messages: List[SystemMessage] = []
        if reflection_system_message is None:
            self._reflection_system_messages = []
        else:
            self._reflection_system_messages = [SystemMessage(content=reflection_system_message)]
    ...
    async def on_messages_stream(
        ...
        reflection_system_messages = self._reflection_system_messages
        ...
        async for output_event in self._process_model_result(
            ...
            reflection_system_messages=reflection_system_messages,
    ...
    async def _process_model_result(
        ...
        model_client_stream: bool,
        reflect_on_tool_use: bool,
        reflection_system_messages: List[SystemMessage],
        tool_call_summary_format: str,
    ) -> AsyncGenerator[AgentEvent | ChatMessage | Response, None]:
        """
    ...
    async def _reflect_on_tool_use_flow(
        ...
        llm_messages = cls._get_compatible_context(
            model_client=model_client, messages=reflection_system_messages + all_messages
        )
